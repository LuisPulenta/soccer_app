...
class _PredictionsScreenState extends State<PredictionsScreen>
            ...
            Tab(
              child: Row(
                ...
                children: const [
                  Icon(Icons.done_all),
                  SizedBox(width: 10),
                  Text('Finalizadas.', style: TextStyle(fontSize: 14)),
                ],
              ),
            ),
          ],
  ...
  _search() async {
    ...
    for (var _completeMatch in _completePredictions) {
      if (_completeMatch.match!.local.initials.toLowerCase().contains(
            _filter.toLowerCase(),
          ) ||
          _completeMatch.match!.visitor.initials.toLowerCase().contains(
            _filter.toLowerCase(),
          ) ||
          _completeMatch.match!.dateName!.toLowerCase().contains(
            _filter.toLowerCase(),
          )) {
  ...
  _search2() async {
    ...
    for (var _pendingMatch in _pendingPredictions) {
      if (_pendingMatch.match!.local.initials.toLowerCase().contains(
            _filter2.toLowerCase(),
          ) ||
          _pendingMatch.match!.visitor.initials.toLowerCase().contains(
            _filter2.toLowerCase(),
          ) ||
          _pendingMatch.match!.dateName!.toLowerCase().contains(
            _filter2.toLowerCase(),
          )) {
        ...
        children: <Widget>[
          Expanded(
            child: ElevatedButton(
              ...
              onPressed: () {
                _localGoalsController.text = e.goalsLocal == null
                    ? ''
                    : e.goalsLocal.toString();

                _visitorGoalsController.text = e.goalsVisitor == null
                    ? ''
                    : e.goalsVisitor.toString();
                                ...
                                onPressed: () async {
                                  if (_localGoalsController.text == '') {
                                    await showAlertDialog(
                                      context: context,
                                      title: 'Error',
                                      message: 'Ingrese Goles para el Local',
                                      actions: <AlertDialogAction>[
